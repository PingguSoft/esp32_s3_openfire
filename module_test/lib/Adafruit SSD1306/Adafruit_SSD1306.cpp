...
/*!
   ...
   protected function, not exposed (see ssd1306_command() instead).
    ...
    @note
*/
uint8_t Adafruit_SSD1306::ssd1306_command1(uint8_t c) {
  if (wire) { // I2C
...
/*!
   ...
   transactions. This is a protected function, not exposed.
    ...
    @note
*/
 uint8_t Adafruit_SSD1306::ssd1306_commandList(const uint8_t *c, uint8_t n) {
  if (wire) { // I2C
    wire->beginTransmission(i2caddr);
    WIRE_WRITE((uint8_t)0x00); // Co = 0, D/C = 0
    uint16_t bytesOut = 1;
...
uint8_t Adafruit_SSD1306::ssd1306_command(uint8_t c) {
  TRANSACTION_START
  uint8_t ret = ssd1306_command1(c);
  TRANSACTION_END
...
bool Adafruit_SSD1306::begin(uint8_t vcs, uint8_t addr, bool reset,
  ...
  uint8_t ret = ssd1306_commandList(init1, sizeof(init1));
  if (ret > 0)
    return false;
